...
import { rootElm, sandboxMap, findTarget, isObject, hasOwn, makeMap } from '../utils';
    ...
    return (target: any, p: PropertyKey, receiver?: any) => {
        ...
        if (rootNode) {
            if (p === 'createElement') {
                ...
                return findTarget(rootNode, ['head', 'div[__garfishmockhead__]']) || value;
            }
            ...
            if (strictIsolation) {
                if (p === 'body') {
                    ...
                    return p === 'getElementById' ? id => rootNode.querySelector(`#${id}`) : rootNode[p].bind(rootNode);
                }
            }
        }
