...
import { Module, SandboxOptions, ExecScriptOptions, ReplaceGlobalVariables } from './types';
...
const isModule = (module: Window) => {
    return isObject(module) ? module[__garfishGlobal__ as any] !== undefined : false;
};
...
export class Sandbox {
    ...
    static canSupport() {
        ...
        if (!support) {
            warn(
                'The current environment does not support "vm sandbox",' + 'Please use the "snapshot sandbox" instead.'
            );
        }
        return support;
    }
